 ...
 *
 */
static uint32_t our_battery_char_add(ble_os_t * p_our_service)
{
    // OUR_JOB: Step 2.A, Add a custom characteristic UUID
		...
		char_uuid.uuid = BLE_UUID_BATTERY_UUID;					
    ...
    // OUR_JOB: Step 2.H, Set characteristic length in number of bytes
		...
		uint8_t value[4]            = {0xAA,0xAA,0xAA, 0xAA};
    ...
    // OUR_JOB: Step 2.E, Add our new characteristic to the service
		err_code = sd_ble_gatts_characteristic_add(p_our_service->service_handle,
                                   &char_md,
                                   &attr_char_value,
                                   &p_our_service->battery_char_handles);
		APP_ERROR_CHECK(err_code);

    return NRF_SUCCESS;
}

static uint32_t our_notes_char_add(ble_os_t * p_our_service)
{
	
		uint32_t err_code;
		ble_uuid_t char_uuid;
		ble_uuid128_t base_uuid = BLE_UUID_OUR_BASE_UUID;
		char_uuid.uuid = BLE_UUID_NOTES_UUID;					
		err_code = sd_ble_uuid_vs_add(&base_uuid, &char_uuid.type);
		APP_ERROR_CHECK(err_code);
...
//		attr_char_value.p_value     = value;
    ...
    // OUR_JOB: Step 2.E, Add our new characteristic to the service
		err_code = sd_ble_gatts_characteristic_add(p_our_service->service_handle,
                                   &char_md,
                                   &attr_char_value,
                                   &p_our_service->notes_char_handles);
		APP_ERROR_CHECK(err_code);
...
void our_service_init(ble_os_t * p_our_service)
{
    ...
    our_notes_char_add(p_our_service);
		//our_battery_char_add(p_our_service);
}
